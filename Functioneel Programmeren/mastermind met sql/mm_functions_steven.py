...
def Choose4Colors(kleuren):
    ...
    recordID = create_new_SQL_record(''.join(antwoord))
    return antwoord, recordID
...
def SolutionFoundEndGame(antwoord):
    print("Proficiat!")
    print( f"Het juiste antwoord was inderdaad: { antwoord }")
...
# hoofdprocedure spel
def PlayGame(antwoord, recordID):
    ...
    while True:
        ...
        if UserWantsToQuitEarly(gok, antwoord):
            update_SQL_afgebroken(goklijst_voor_SQL, recordID)
            break
        else:
